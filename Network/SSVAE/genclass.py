...
import numpy as np
import Util.vae_utils as utils
import time

from Network.neuralnetworks import FullyConnected
from prettytensor import bookkeeper
import params as par
import Util.gen_class_utils as gen_util

class GenerativeClassifier( object ):

	def __init__(   self,
					dim_x, dim_z, dim_y,
					# num_examples=1, num_lab=1, num_batches=1,
					p_x = 'gaussian',
					q_z = 'gaussian_marg',
					p_z = 'gaussian_marg',
		...
		with self.G.as_default():
			...
			self.x_mu 		= tf.placeholder( tf.float32, [None, self.dim_x] )
			self.x_lsgms 		= tf.placeholder( tf.float32, [None, self.dim_x] )
	...
	def _generate_zxy( self, x, y, reuse = False ):
		...
		with tf.variable_scope('encoder', reuse = reuse):
			encoder_out = self.encoder.output( tf.concat( [x, y], 1 ) ) #TODO: fix?
	...
	def _generate_xzy( self, z, y, reuse = False ):
		...
		with tf.variable_scope('decoder', reuse = reuse):
			decoder_out = self.decoder.output( tf.concat( [z, y], 1 ) ) #TODO: fix?
	...
	def _objective( self ):
		...
		self.y_lab_logits, self.x_lab = self._generate_yx( self.x_mu, self.x_lsgms )
		...
		for label in range(self.dim_y):
			...
			else: L_ulab = tf.concat( [L_ulab, _L_ulab], 1 ) #TODO: erase?, _Lulab vs Lulab
	...
	def test(self, x_test):
		...
		predict_label = \
			self.session.run([tf.argmax(self.y_ulab)],
							 feed_dict={self.x_unlabelled_mu: x_test_mu, self.x_unlabelled_lsgms: x_test_lsgms,
										})
