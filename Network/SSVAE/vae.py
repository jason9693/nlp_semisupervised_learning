...
class VariationalAutoencoder(object):
	...
	def _objective( self ):
		...
		print(post_z.shape, prior_z.shape, self.log_lik.shape, l2.shape)
		...
		with self.session as sess:
			...
			for epoch in range(epochs):
				...
				for x_batch in utils.feed_numpy( self.batch_size, x ):
					...
					#print(sess.run(self.z_sample,feed_dict = { self.x: x_batch }))
				...
				if stop_counter >= stop_iter:
					print('Stopping VAE training in {} epochs'.format(epoch))
